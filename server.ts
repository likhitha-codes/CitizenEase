...
// Initialize Gemini SDK with telemetry headers as per rules
...
async function startServer() {
  ...
  app.post('/api/auth/register', (req, res) => {
    ...
    const newUser = {
      ...
      password, // Plain for simplicity in preview, can hash but simplicity is key here
      fullName,
      token,
  ...
  app.post('/api/process', async (req, res) => {
    ...
    try {
      ...
      if (fileBase64 && mimeType) {
        // If file is provided (PDF, JPG, PNG etc.)
        contents.push({
          inlineData: {
            mimeType: mimeType,
            data: fileBase64
...
IMPORTANT: Your response must be returned STRICTLY as a valid JSON object matching the defined response Schema type. Keep the formatting completely correct with no trailing commas.`
      ...
      // If requested by a logged-in user (optional token sent in headers), we can log to history automatically
      const authHeader = req.headers.authorization;
      if (authHeader && authHeader.startsWith('Bearer ')) {
        const token = authHeader.split(' ')[1];
        const db = readDB();
  ...
  app.get('/api/history', requireAuth, (req: any, res) => {
    ...
    res.json(userHistory.slice(0, 30)); // Limit to last 30
  });
  ...
  // Mount Vite middleware for asset serving in Development, and production logic
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[DocuEase Express Server] Running on http://localhost:${PORT}`);
  });
}
